...
if ! [ -e /init/built_servo_once_successfully ]; then
    ...
    ./mach build --use-crown --locked --release  -v 2>&1 | tee /init/build0.log
    # Some hacks that seem to help with incremental builds.
    git status
    ./mach build --use-crown --locked --release  -v 2>&1 | tee /init/build1.log
    ...
    export CARGO_LOG=cargo::core::compiler::fingerprint=info
    ./mach build --use-crown --locked --release  -v 2>&1 | tee /init/build2.log
fi
